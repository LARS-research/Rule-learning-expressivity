...
class NeuralBellmanFordNetwork(nn.Module, core.Configurable):
    ...
    def __init__(self, input_dim, hidden_dims, num_relation=None, symmetric=False,
                 ...
                 num_beam=10, path_topk=10, degree=1000):
        ...
        self.entity_embedding = nn.Embedding(2200 , input_dim)
    ...
    def bellmanford(self, graph, h_index, r_index, selected_embed_idx, separate_grad=False):
        query = self.query(r_index)
        index = h_index.unsqueeze(-1).expand_as(query)
        boundary = torch.zeros(graph.num_node, *query.shape, device=self.device)
        selected_embed = self.entity_embedding(torch.arange(len(selected_embed_idx), device=self.device))
        boundary[selected_embed_idx, :, :].add_(selected_embed.unsqueeze(1))
    ...
    def forward(self, graph, h_index, t_index, r_index=None, all_loss=None, metric=None):
        ...
        out_degree = graph.degree_out
        selected_embed_idx = torch.where(out_degree > self.degree)[0]
        # index_map = torch.zeros(graph.num_node, dtype=torch.long, device=self.device)
        # index_map[selected_embed_idx] = torch.arange(selected_embed_idx.shape[0], device=self.device)
        # if selected_embed_idx.shape[0] == 0:
        #     print('no point')
            # exit()
        ...
        output = self.bellmanford(graph, h_index[:, 0], r_index[:, 0], selected_embed_idx)
        ...
        if self.symmetric:
            ...
            output = self.bellmanford(graph, t_index[:, 0], r_index[:, 0], selected_embed_idx)
    ...
    def visualize(self, graph, h_index, t_index, r_index, selected_embed_idx):
        assert h_index.numel() == 1 and h_index.ndim == 1
        graph = graph.undirected(add_inverse=True)

        output = self.bellmanford(graph, h_index, r_index, selected_embed_idx, separate_grad=True)
